...
import { Shield, AlertTriangle, CheckCircle, Search, ExternalLink, Globe, Lock, Eye } from 'lucide-react';
import { checkWebsiteSecurity, SecurityCheckResult } from '../utils/webChecker';
import { checkScamDomain, getAllScamDomains } from '../utils/blockchain';
import { addSecurityAlert, addActivityLog, getCurrentUser } from '../utils/storage';

const EnhancedScamChecker: React.FC = () => {
  const [url, setUrl] = useState('');
  const [isChecking, setIsChecking] = useState(false);
  const [result, setResult] = useState<SecurityCheckResult | null>(null);
  const [blockchainResult, setBlockchainResult] = useState<boolean | null>(null);
  ...
  const handleCheck = async () => {
    ...
    try {
      // Check with our security engine
      const checkResult = await checkWebsiteSecurity(url);
      setResult(checkResult);
      
      // Check against blockchain registry
      try {
        const domain = extractDomain(url);
        const isScamOnChain = await checkScamDomain(domain);
        setBlockchainResult(isScamOnChain);
        
        // If blockchain says it's a scam, update our result
        if (isScamOnChain) {
          checkResult.status = 'dangerous';
          checkResult.riskScore = Math.max(checkResult.riskScore, 90);
          checkResult.threats.push('Listed in Blockchain Scam Registry');
        }
      } catch (blockchainError) {
        console.warn('Blockchain check failed:', blockchainError);
      }
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'safe': return 'text-green-400 bg-green-400/20 border-green-400/30';
      case 'warning': return 'text-yellow-400 bg-yellow-400/20 border-yellow-400/30';
      case 'dangerous': return 'text-red-400 bg-red-400/20 border-red-400/30';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 text-center">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">Blockchain Registry</h3>
            <p className="text-gray-400 text-sm">Verified against on-chain scam database</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 text-center">
            <Eye className="w-12 h-12 text-green-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-white mb-2">AI Analysis</h3>
            <p className="text-gray-400 text-sm">Advanced pattern recognition</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 text-center">
            <Lock className="w-12 h-12 text-purple-400 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-white mb-2">Real-time Protection</h3>
            <p className="text-gray-400 text-sm">Instant threat detection</p>
          </div>
        </div>
        ...
        {result && (
          <div className="bg-gray-900 rounded-xl p-8 border border-gray-800 mb-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="text-2xl font-semibold text-white mb-2">Enhanced Security Analysis</h3>
            ...
            {blockchainResult !== null && (
              <div className="mb-6 p-4 bg-gray-800 rounded-lg border border-gray-700">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="text-white font-semibold">Blockchain Verification</h4>
                    <p className={`text-sm ${blockchainResult ? 'text-red-400' : 'text-green-400'}`}>
                      {blockchainResult 
                        ? '⚠️ Listed in blockchain scam registry' 
                        : '✅ Not found in scam registry'
                      }
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-800 rounded-lg p-6">
                  ...
                  <div className="relative">
                    <div className="w-full bg-gray-700 rounded-full h-6">
                      <div 
                        className={`h-6 rounded-full transition-all duration-1000 ${
                          result.riskScore > 70 ? 'bg-gradient-to-r from-red-500 to-red-600' : 
                          result.riskScore > 40 ? 'bg-gradient-to-r from-yellow-500 to-orange-500' : 
                    ...
                    <div className="flex justify-between text-sm text-gray-400 mt-2">
                      <span>Safe</span>
                      <span className="font-bold text-white text-lg">{result.riskScore}/100</span>
                      <span>Dangerous</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className={
                        ...
                        {result.details.reputation.toUpperCase()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400">Malware Detected:</span>
                      <span className={result.details.malwareDetected ? 'text-red-400' : 'text-green-400'}>
                        {result.details.malwareDetected ? '⚠️ Yes' : '✅ No'}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  {result.threats.length > 0 ? (
                    <div className="space-y-2">
                      {result.threats.map((threat, index) => (
                        <div key={index} className="flex items-center space-x-2 text-red-400 p-2 bg-red-500/10 rounded">
                ...
                <div className="bg-gray-800 rounded-lg p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-300">
                    {result.status === 'dangerous' && (
                      <>
                        <p className="text-red-400">🚫 Do not enter personal information</p>
                        <p className="text-red-400">🚫 Avoid downloading files</p>
                        <p className="text-red-400">🚫 Report this site if suspicious</p>
                      </>
                    )}
                    {result.status === 'warning' && (
                      <>
                        <p className="text-yellow-400">⚠️ Exercise caution</p>
                        <p className="text-yellow-400">⚠️ Verify site legitimacy</p>
                        <p className="text-yellow-400">⚠️ Check URL carefully</p>
                      </>
                    )}
                    {result.status === 'safe' && (
                      <>
                        <p className="text-green-400">✅ Site appears legitimate</p>
                        <p className="text-green-400">✅ SSL certificate valid</p>
                        <p className="text-green-400">✅ No known threats detected</p>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="mt-6 p-4 bg-gray-800 rounded-lg">
              <p className="text-sm text-gray-400">
                ...
                Results verified with blockchain registry • 
                      ...
                      <div>
                        ...
                        {check.threats.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2">
                            {check.threats.slice(0, 3).map((threat, i) => (
...
export default EnhancedScamChecker;